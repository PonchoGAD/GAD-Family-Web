...
const normAddr = (s: string) => {
  ...
  try { return getAddress(t); } catch { return t; }
};
...
export default function FarmingDashboard() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const res2 = await fetch('/farming.config.json', { cache: 'no-store' }).catch(() => null as Response | null);
  ...
  const connect = async () => {
    ...
    try {
      ...
      setErr(er?.message || 'Failed to connect wallet');
    }
  };
  ...
  const handleSwitch = async () => {
    try {
      ...
      const er = e as { message?: string };
      setErr(er?.message || 'Failed to switch network');
    }
  };
...
function PoolCard({
  ...
  const rewardPerBlockBig = BigInt(cfg.rewardPerBlock);
  ...
  const blocksPerDayBig   = 28800n;  const perDayBig = poolPerBlockBig * blocksPerDayBig;
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      setMsg(er?.shortMessage || er?.message || 'Failed to refresh');
    }
  ...
  const approveMax = async () => {
    ...
    try {
      ...
      const tx = await lp.approve(chefAddr, parseUnits('1000000000000', lpDecimals));
      await tx.wait();
      setMsg('Approved');
      await refresh();
    } catch (e) {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
      ...
      <div className="mt-5 flex gap-2">
        ...
        />
        {needApprove ? (
          <button onClick={approveMax} className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 disabled:opacity-50" disabled={disabled || busy}>Approve</button>
        ) : (
          <button onClick={deposit} className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 disabled:opacity-50" disabled={disabled || busy}>Stake</button>
        )}
        <button onClick={withdraw} className="px-4 py-2 rounded-xl border border-white/20 hover:border-white/40 disabled:opacity-50" disabled={disabled || busy}>Unstake</button>
        <button onClick={harvest} className="px-4 py-2 rounded-xl bg-[#ffd166] text-[#0b0f17] font-semibold hover:opacity-90 disabled:opacity-50" disabled={disabled || busy}>Harvest</button>
      </div>
