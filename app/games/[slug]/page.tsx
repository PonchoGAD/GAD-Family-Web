...
export const dynamic = "force-static"; // страница статическая, данные берём из public
...
export default function GameDetailPage({ params }: Props) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      <Link href="/games" className="opacity-80 hover:opacity-100">← All games</Link>
      <h1 className="mt-4 text-3xl font-semibold">{game.title}</h1>
      <p className="mt-2 opacity-80">Reward: {game.reward?.GAD ?? 0} GAD • Cooldown: {game.cooldownHours ?? 0}h</p>

      <div className="mt-6">
        <button onClick={handleClaim} disabled={claiming} className="rounded-xl border border-white/10 px-4 py-3 hover:bg-white/5 disabled:opacity-60">
