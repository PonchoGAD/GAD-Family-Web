...
function useToast() {
  ...
  const show = React.useCallback((msg: string, ms = 2500) => {
    setToast(msg);
    setTimeout(() => setToast(null), ms);
  }, []);
  const node = toast ? (
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 rounded-xl bg-white/90 text-black px-4 py-2 text-sm shadow-xl">
      {toast}
    </div>
  ) : null;
  return { show, node };
...
function MintBox({ image, onMinted }: { image: string | null; onMinted: (tokenId: string) => void }) {
  ...
  const [explorerBase, setExplorerBase] = React.useState<string>("https://bscscan.com");
  ...
  const doMint = async () => {
    ...
    try {
      ...
      try {
        ...
        setExplorerBase(getExplorerBase(Number(net.chainId)));
      } catch {
        setExplorerBase("https://bscscan.com");
      }
      ...
      const gasPrice =
        feeData.gasPrice && feeData.gasPrice > 0n ? feeData.gasPrice : ethers.parseUnits("3", "gwei");

      let gasLimit: bigint = 300000n;
      try {
        const est = await signer.estimateGas({
          to: ADDR.NFT721,
          data: new ethers.Interface(nft721Abi).encodeFunctionData("mintWithFee", [
      ...
      // симуляция (+ from) — помогает выяснить revert до отправки
      try {
        await provider.call({ ...txReq, from: to });
      } catch {
        // не блокируем — некоторые RPC не возвращают reason
      }
      ...
      show(`Minted${mintedTokenId ? ` #${mintedTokenId}` : ""} ✅`);

      // опционально — alert для мгновенной обратной связи
      alert(
        `NFT minted!${mintedTokenId ? ` Token #${mintedTokenId}` : ""}${
          tx.hash ? `\nTx: ${explorerBase}/tx/${tx.hash}` : ""
        }`
      );
    } catch (e: unknown) {
  ...
  return (
    <>
      <div className="border rounded-xl p-4 bg-[#0E0E12]/80 text-white space-y-3">
        ...
        {/* ссылки на explorer после отправки/минта */}
        {(txHash || status.startsWith("Minted")) && (
          <div className="text-xs mt-2 space-y-1">
            {txHash && (
              <a
                className="text-[#FFD166] underline"
            ...
            <div>
              <a
                className="text-[#FFD166] underline"
                href={`${explorerBase}/token/${ADDR.NFT721}`}
                target="_blank"
                rel="noopener noreferrer"
              >
