...
// Минимальные типы ответа BscScan logs API
type BscScanLog = {
  topics: string[];
  data: string;
};
type BscScanLogsResponse =
  | { status: "1"; result: BscScanLog[] }
  | { status: "0"; result: unknown };
...
export async function GET() {
  try {
    ...
    const data = (await resp.json()) as BscScanLogsResponse;
    ...
    if (data.status === "1" && Array.isArray(data.result)) {
      ...
      for (const log of data.result) {
        if (Array.isArray(log.topics) && log.topics.length >= 2) {
          const t1 = log.topics[1]; // 0x000...<20 bytes address>
    ...
    const message = e instanceof Error ? e.message : "failed to load staking stats";
